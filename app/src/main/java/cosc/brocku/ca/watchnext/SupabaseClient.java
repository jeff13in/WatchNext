...
public class SupabaseClient {
    ...
    public static void getUser(String firebaseUid, DataCallback callback) {
        executor.execute(() -> {
            try {
                String endpoint = "users?firebase_uid=eq." + encode(firebaseUid) + "&limit=1";
    ...
    public static void updateMood(String firebaseUid, String mood, Callback callback) {
        executor.execute(() -> {
            try {
                JSONObject body = new JSONObject();
                body.put("mood", mood);
                patch("users?firebase_uid=eq." + encode(firebaseUid), body, callback);
            } catch (Exception e) {
                callback.onFailure(e.getMessage());
            }
        });
    ...
    public static void getWatchlist(int userId, ListCallback callback) {
        executor.execute(() -> {
            try {
                String endpoint = "watchlist?user_id=eq." + userId + "&order=added_at.desc,id.desc";
                HttpURLConnection conn = openConnection(BASE_URL + endpoint, "GET");
                String response = readResponse(conn);
                conn.disconnect();

                JSONArray raw = new JSONArray(response);
                JSONArray deduped = dedupeByMovieId(raw);
     ...
     * - if rows already exist for this user/movie, keep the newest one
     * - update the kept row
        ...
        executor.execute(() -> {
            try {
                ...
                if (existing.length() > 0) {
                    int keepId = existing.getJSONObject(0).getInt("id");
                    ...
                    patchSync("watchlist?id=eq." + keepId, body);

                    // delete duplicate older rows if any
                    for (int i = 1; i < existing.length(); i++) {
                        int duplicateId = existing.getJSONObject(i).getInt("id");
                        deleteSync("watchlist?id=eq." + duplicateId);
                    }
    ...
    public static void getRatings(String movieId, ListCallback callback) {
        executor.execute(() -> getList("ratings?movie_id=eq." + encode(movieId) + "&order=created_at.desc", callback));
    }
        ...
        executor.execute(() -> {
            try {
                ...
                if (existing.length() > 0) {
                    ...
                    patchSync("watch_history?id=eq." + keepId, body);

                    // delete duplicate older rows if any
                    for (int i = 1; i < existing.length(); i++) {
                        int duplicateId = existing.getJSONObject(i).getInt("id");
                        deleteSync("watch_history?id=eq." + duplicateId);
                    }
    ...
    public static void getWatchHistory(int userId, ListCallback callback) {
        executor.execute(() -> {
            try {
                String endpoint = "watch_history?user_id=eq." + userId + "&order=watched_at.desc,id.desc";
                HttpURLConnection conn = openConnection(BASE_URL + endpoint, "GET");
                String response = readResponse(conn);
                conn.disconnect();
    ...
    public static void getReview(int userId, String movieId, ListCallback callback) {
        executor.execute(() -> getList(
                "reviews?user_id=eq." + userId + "&movie_id=eq." + encode(movieId) + "&limit=1",
                callback
        ));
    }
    ...
    private static void patchSync(String endpoint, JSONObject body) throws Exception {
    ...
    private static void deleteSync(String endpoint) throws Exception {
    ...
    private static JSONArray dedupeByMovieId(JSONArray raw) {
