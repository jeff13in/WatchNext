...
public class ListEntryAdapter extends RecyclerView.Adapter<ListEntryAdapter.ViewHolder> {
    ...
    public interface OnEntryChangedListener {
        void onEntryChanged(ListEntry entry);
    }
    ...
    public ListEntryAdapter(List<ListEntry> entries,
                            OnEntryChangedListener listener,
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        switch (entry.getStatus()) {
            case "Finished":
                indicatorColor = R.color.btn_watched;
                break;
            case "Liked":
                indicatorColor = R.color.btn_like;
                break;
            case "Disliked":
                indicatorColor = R.color.btn_dislike;
                break;
            default:
                indicatorColor = R.color.btn_watchlist;
                break;
        }
            ...
            holder.btnEpisode.setOnClickListener(v -> {
                ...
                if (entry.getSupabaseId() != -1) {
                    v.getContext().getSharedPreferences("ep_progress", Context.MODE_PRIVATE)
                            .edit()
                            .putInt("ep_" + entry.getSupabaseId(), next)
                            .apply();
                }
            });
        ...
        SupabaseClient.removeFromWatchlist(entry.getSupabaseId(),
                new SupabaseClient.Callback() {
                    ...
                    public void onSuccess() {
                        v.getContext().getSharedPreferences("ep_progress", Context.MODE_PRIVATE)
                                .edit()
                                .remove("ep_" + entry.getSupabaseId())
                                .apply();
